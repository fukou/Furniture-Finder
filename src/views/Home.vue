<template>
  <div>
    <v-row align="center" class="px-3">
      <v-col cols="12" sm="12">
        <v-text-field v-model="search" label="Search furniture"></v-text-field>
        SelectItem : {{ selectedDelivery }}
      </v-col>
      <v-col cols="12" sm="6">
        <v-combobox
    ...
    <v-container fluid grid-list-lg>
      <v-layout row wrap>
        ...
        <v-flex else sm6 v-for="(product, index) in filteredItems" :key="index">
          <v-card class="mx-auto pa-3">
            <v-card-text>
              ...
              <div class="text--primary">{{ product.description | truncate(114, "...") }}</div>
              ...
              <div class="mt-5 d-flex justify-space-between align-center">
                ...
                <p
                  class="font-weight-bold indigo--text text--darken-1"
                >{{ product.delivery_time }} days</p>
              </div>
            </v-card-text>
          </v-card>
        </v-flex>
